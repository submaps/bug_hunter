...
game.prototype.setup = function()
{
	...
	//the player
	this.player = new player({x : w/2+w/5, y: h/2 , 
							  game : this, player_name : "neo1", 
							  player_img : "neo1.png", lookleft: true});
	this.game_objects.push(this.player);
	
	this.player2 = new player({x : w/2-w/5, y: h/2 , 
							   game : this, 
							   player_name : "neo2", 
							   player_img : "neo2.png", lookleft: false});
...
game.prototype.key_up = function(e)
{
	...
	{
		this.player2.do_kick = !this.player2.do_kick;
		this.player2.img = this.player2.player_img;
	}
	// ENTER
	else if(code == 13)
	{
		this.player.do_kick = !this.player.do_kick;
		this.player.img = this.player.player_img;
	}
}
...
function player(options)
{
	...
	this.do_kick = false;
	this.lookleft = options.lookleft;

	this.max_hor_vel = 2;
	this,max_ver_vel = 4;
	this.can_move_up = true;
	this.player_img = img_res(options.player_img);
	// alert("player_img: "+this.player_img + " player_name: "+this.player_name);
	this.img = this.player_img;
	var info = {
		'density' : 10 ,
		'fixedRotation' : true ,
		'userData' : this ,
...
player.prototype.tick = function()
{
	...
	if(this.do_kick){
		// alert("do_kick player2");
		if (this.lookleft)
			this.img = player.img_kick_left;
		else this.img = player.img_kick_right;
	}	else if(this.do_move_left)
	{
		this.add_velocity(new b2Vec2(-1,0));
		this.img = player.img_left;
	}
	else if(this.do_move_right)
	{
		this.add_velocity(new b2Vec2(1,0));
		this.img = player.img_right;
	} //else (this.do_k)
	// 	// this.img = choose([player.img_left, player.img_right]);
	// 	this.img = this.player_img;
...
player.img_left = img_res('neo1.png');
player.img_right = img_res('neo2.png');
player.img_kick_left = img_res('neo_kick_left.png');
player.img_kick_right = img_res('neo_kick_right.png');
